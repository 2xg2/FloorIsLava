...
public class LineBase2 : MonoBehaviour
{
    ...
    public float lifeTime = 10.0f;                   //the amount of time in seconds that the trail lasts
    ...
    public bool colliderIsTrigger = false;           //determines if the collider is a trigger.  Changing this during runtime will have no effect.
    ...
    private void Awake()
    {
        ...
        gameObject.GetComponent<Renderer>().material = trailMaterial;
        ...
        centerPositions.AddFirst(gameObject.transform.position);
        centerPositions.AddFirst(gameObject.transform.position + new Vector3(0.1f, 0f, 0f));

        leftVertices = new LinkedList<Vertex>();
        rightVertices = new LinkedList<Vertex>();

        //headTransform = new GameObject("head").transform;
        //headTransform.transform.parent = gameObject.transform;
    }
    ...
    /// </summary>
    public bool TryAddVertices(Vector3 newPos)
    {
        //Debug.Log("TryAddVetices: newPos = " + newPos);
        ...
        //if ((centerPositions.First.Value - newPos).sqrMagnitude > vertexDistanceMin * vertexDistanceMin)
        {
            //calculate the normalized direction from the 1) most recent position of vertex creation to the 2) current position
            Vector3 dirToCurrentPos = (newPos - centerPositions.First.Value).normalized;
            ...
            Vector3 leftPos = newPos + (cross * -widthStart * 0.5f);
            Vector3 rightPos = newPos + (cross * widthStart * 0.5f);

            //create two new vertices at the calculated positions
            leftVertices.AddFirst(new Vertex(leftPos, newPos, (leftPos - newPos).normalized));
            rightVertices.AddFirst(new Vertex(rightPos, newPos, (rightPos - newPos).normalized));

            //add the current position as the most recent center position
            centerPositions.AddFirst(newPos);
    ...
    {
        return centerPositions.First.Value;
    }
    ...
    public Vector3 GetHeadDirection()
    {
        ...
        Vector3 p1 = centerPositions.First.Next.Value;
        Vector3 p2 = centerPositions.First.Value;
